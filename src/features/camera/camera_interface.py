...
from src.services.data.signals import DrawViewSignalManager, CameraSignalManager
from src.features.devices_submenu import CameraSelectorWidget
...
class CameraInterface(QWidget):
    ...
    def __setup_ui(self):
        ...
        self.image_path_d = "img:camera_d.svg"
        self.image_path_l = "img:camera_l.svg"
    ...
    def __setup_connections(self):
        ...
        self.theme_manager.theme_changed.connect(self.image_handler.update_theme)
    ...
    def _get_selected_camera_name(self):
        if self.last_camera_name:
            return self.last_camera_name
        return None
    ...
    def on_frame_ready(self, frame: np.ndarray):
        ...
        if self.process_running and frame is not None:
            ...
            if isinstance(frame, np.ndarray):
                pixmap = ImageHandler.numpy_to_qpixmap(frame)
            elif isinstance(frame, cv2.UMat):
                pixmap = ImageHandler.umat_to_pixmap(frame)
    ...
    def start_video(self):
        ...
        try:
            ...
            self.image_handler.set_process_running(True)
    ...
    def stop_video(self):
        ...
        """
        self.process_running = False
        self.image_handler.set_process_running(False)
